...
import os # Imported for setting CHROMA_DISABLE_TELEMETRY if ChromaDB is used
...
def run_rag_sequence(query: str, nn_index, metadata_list: list, chunk_folder: Path, 
    ...
    # Decode only the newly generated tokens (excluding the prompt part)
...
if __name__ == "__main__":
    ...
    from config import EMBEDDED_CHUNKS_DIR, INDEXED_DATA_DIR, EMBEDDING_MODEL_NAME, LLM_MODEL_NAME, DEVICE, CHUNKS_DIR
    
    print("Running RAG System module directly (for testing)...")
    ...
    if not index_path.exists() or not metadata_path.exists():
        ...
        try:
            response = run_rag_sequence(
                ...
                EMBEDDED_CHUNKS_DIR, # Pass EMBEDDED_CHUNKS_DIR as chunk_folder for retrieval
