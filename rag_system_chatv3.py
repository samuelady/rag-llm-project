...
def build_rag_prompt(query: str, hits: list[dict]) -> str:
    ...
    Refined for concise summarization and strict adherence to context.
    """
    # Enhanced prompt to encourage summarization and synthesis from multiple sources
    prompt = (
        "You are an expert in cybersecurity. Synthesize a concise answer to the following question "
        "by combining and summarizing information from the provided excerpts. "
        "Each distinct piece of information (sentence or phrase) in your answer MUST be followed by "
        "its source citation in the format [Source: doc_id_chunk_id]. "
        "Do NOT include redundant information. Be direct and avoid conversational filler. "
        "If the complete answer is not available in the provided documents, "
        "state 'The information is not fully available in the provided documents.' "
...
def run_rag_sequence(query: str, nn_index, metadata_list: list, chunk_folder: Path,
    ...
    with torch.no_grad(): # Disable gradient calculations for inference
        outputs = llm_model.generate(
            **inputs,
            max_new_tokens=150, # Adjusted for conciseness, can be further reduced if needed
            ...
            # num_beams=3,        # Optional: Can uncomment for more focused answers, but might be slower
            # early_stopping=True # Use with num_beams for efficiency
        )
