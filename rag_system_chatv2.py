...
def build_rag_prompt(query: str, hits: list[dict]) -> str:
    ...
    Refined for conciseness and strict adherence to context.
    """
    # Emphasize conciseness and strict adherence to provided text
    prompt = (
        ...
        "at the end of each relevant sentence or phrase. If the information is not available "
        "in the provided documents, state 'The information is not available in the provided documents.'. "
...
def run_rag_sequence(query: str, nn_index, metadata_list: list, chunk_folder: Path,
    ...
    with torch.no_grad(): # Disable gradient calculations for inference
        outputs = llm_model.generate(
            **inputs,
            max_new_tokens=150, # Reduced max_new_tokens for conciseness
            temperature=0.3,    # Reduced temperature for less creativity, more factual adherence
            top_p=0.9,          # Kept high, but might adjust with temperature
            do_sample=True,     # Keep sampling to avoid repetitive text, but controlled by temp/top_p
            pad_token_id=llm_tokenizer.eos_token_id,
            repetition_penalty=1.1, # Penalize repetition
            num_return_sequences=1,
            # num_beams=5,        # Optional: Uncomment and use for beam search, often yields better quality but slower
